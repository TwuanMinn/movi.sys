      ...
      >
        <div className="analytics-two-col">
          ...
          <motion.div variants={pageVariants.analytics.item} style={S.card}>
            <h4 style={{ color: "#e1e2eb", fontFamily: "var(--font-display, 'Manrope', serif)", fontSize: 18, margin: "0 0 16px" }}>Revenue vs Budget</h4>
            {movies.map(m => (
              <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, flexWrap: "wrap", gap: 4 }}>
                    <span style={{ color: "#e1e2eb", fontSize: 14 }}>{m.title}</span>
                    <span style={{ color: m.revenue > m.budget ? "#82dab0" : "#D4903B", fontSize: 12 }}>{m.revenue > 0 ? `${((m.revenue / m.budget) * 100).toFixed(0)}% ROI` : "—"}</span>
                  </div>
                  <div style={{ display: "flex", gap: 2, height: 8 }}>
                    <div style={{ width: `${(m.budget / 500) * 100}%`, background: "rgba(212,83,75,0.35)", borderRadius: "3px 0 0 3px" }} />
                    <div style={{ width: `${(m.revenue / 500) * 100}%`, background: "#82dab0", borderRadius: "0 3px 3px 0" }} />
                  </div>
                </div>
              </div>
            ))}
          ...
          {/* Campaign Spend */}
          <motion.div variants={pageVariants.analytics.item} style={S.card}>
            <h4 style={{ color: "#e1e2eb", fontFamily: "var(--font-display, 'Manrope', serif)", fontSize: 18, margin: "0 0 16px" }}>Campaign Spend Mix</h4>
              ...
              return (
                <div key={m.id} style={{ marginBottom: 16 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5, flexWrap: "wrap", gap: 4 }}>
                    <span style={{ color: "#e1e2eb", fontSize: 14 }}>{m.poster} {m.title}</span>
                    <span style={{ color: "#8b90a0", fontSize: 12 }}>${total.toFixed(1)}M</span>
                  </div>
        ...
        <motion.div variants={pageVariants.analytics.item} style={S.card} className="analytics-heatmap">
          <h4 style={{ color: "#e1e2eb", fontFamily: "var(--font-display, 'Manrope', serif)", fontSize: 18, margin: "0 0 16px" }}>Engagement Heatmap</h4>
          <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 5 }}>
            <thead><tr>
              ...
              {["Social", "Press", "Trailers", "Pre-sales", "Merch", "Streaming"].map(h => (
                <th key={h} style={{ color: "#8b90a0", fontSize: 11, textAlign: "center", padding: "6px 6px" }}>{h}</th>
              ))}
            </tr></thead>
            <tbody>
              {movies.map(m => {
                ...
                return (
                  <tr key={m.id}>
                    <td style={{ color: "#e1e2eb", fontSize: 13, padding: "6px 10px", whiteSpace: "nowrap" }}>{m.poster} {m.title}</td>
                    {vals.map((v, i) => (
                      <td key={i} style={{ textAlign: "center", padding: 5, background: v > 80 ? "rgba(59,165,92,0.2)" : v > 50 ? "rgba(212,144,59,0.15)" : v > 20 ? "rgba(212,83,75,0.12)" : "rgba(255,255,255,0.02)", borderRadius: 6, color: v > 80 ? "#82dab0" : v > 50 ? "#D4903B" : v > 20 ? "#D4534B" : "#3A362F", fontSize: 13, fontWeight: 600 }}>
                        {v > 0 ? v : "—"}
                      </td>
                    ))}
                  </tr>
        ...
        <div className="analytics-rev-grid">
          <motion.div variants={pageVariants.analytics.item} style={S.card}>
            <h4 style={{ color: "#e1e2eb", fontFamily: "var(--font-display, 'Manrope', serif)", fontSize: 18, margin: "0 0 14px" }}>Monthly Revenue Trend</h4>
            <div style={{ display: "flex", alignItems: "flex-end", gap: 5, height: 120 }}>
              {[12, 18, 32, 55, 88, 105, 72, 45, 68, 95, 110, 85].map((v, i) => (
                <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
                  <span style={{ color: "#8b90a0", fontSize: 10 }}>${v}M</span>
                  ...
                  <span style={{ color: "#414755", fontSize: 10 }}>{["J", "F", "M", "A", "M", "J", "J", "A", "S", "O", "N", "D"][i]}</span>
                </div>
              ))}
            </div>
          </motion.div>
          <div className="analytics-genre-roi">
            {[{ g: "Action", r: 180, c: "#D4534B" }, { g: "Drama", r: 206, c: "#4A90D9" }, { g: "Horror", r: 0, c: "#82dab0" }, { g: "Romance", r: 0, c: "#D4903B" }].map(x => (
              <div key={x.g} style={{ background: `${x.c}08`, border: `1px solid ${x.c}18`, borderRadius: 10, padding: 14, textAlign: "center" }}>
                ...
                <div style={{ color: "#e1e2eb", fontSize: 22, fontFamily: "var(--font-display, 'Manrope', serif)", fontWeight: 700 }}>{x.r > 0 ? `${x.r}%` : "—"}</div>
                <div style={{ color: "#8b90a0", fontSize: 11 }}>ROI</div>
              </div>
            ))}
          </div>
        </div>
