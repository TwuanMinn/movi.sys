...
export default function SettingsPage() {
  ...
  function Toggle({ checked, onChange, label }: { checked: boolean; onChange: () => void; label: string }) {
    return (
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid rgba(214,198,170,0.04)" }}>
        <span style={{ color: "#D6C6AA", fontSize: 14 }}>{label}</span>
        <button onClick={onChange} style={{ width: 44, height: 24, borderRadius: 12, border: "none", cursor: "pointer", background: checked ? "rgba(59,165,92,0.3)" : "rgba(255,255,255,0.08)", position: "relative", transition: "background .2s ease" }}>
          <div style={{ width: 18, height: 18, borderRadius: "50%", position: "absolute", top: 3, left: checked ? 23 : 3, background: checked ? "#3BA55C" : "#5A5347", transition: "left .2s ease, background .2s ease" }} />
        </button>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div style={{ padding: "24px 32px 32px", display: "flex", flexDirection: "column", gap: 22 }}>
        {/* Profile */}
        <div style={S.card}>
          <h4 style={{ color: "#D6C6AA", fontFamily: "var(--font-display, 'Cormorant Garamond', serif)", fontSize: 18, margin: "0 0 18px" }}>Profile Settings</h4>
          <div style={{ display: "flex", alignItems: "center", gap: 18, marginBottom: 20, flexWrap: "wrap" }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ color: "#D6C6AA", fontSize: 20, fontFamily: "var(--font-display, 'Cormorant Garamond', serif)", fontWeight: 600 }}>Sarah Chen</div>
              <div style={{ color: "#7A7062", fontSize: 13, marginTop: 2 }}>Director · sarah@studio.com</div>
              <div style={{ color: "#5A5347", fontSize: 12, marginTop: 4 }}>Member since January 2024</div>
            </div>
        ...
        <div className="settings-grid">
          <div className="settings-section">
            <div style={S.card}>
              <h4 style={{ color: "#D6C6AA", fontFamily: "var(--font-display, 'Cormorant Garamond', serif)", fontSize: 16, margin: "0 0 14px" }}>Notification Channels</h4>
            ...
            <div style={S.card}>
              <h4 style={{ color: "#D6C6AA", fontFamily: "var(--font-display, 'Cormorant Garamond', serif)", fontSize: 16, margin: "0 0 14px" }}>Alert Preferences</h4>
          ...
          <div className="settings-section">
            <div style={S.card}>
              <h4 style={{ color: "#D6C6AA", fontFamily: "var(--font-display, 'Cormorant Garamond', serif)", fontSize: 16, margin: "0 0 14px" }}>Appearance</h4>
              <div style={{ marginBottom: 14 }}>
                <label style={S.label}>Theme</label>
                <div style={{ display: "flex", gap: 8 }}>
                  {[{ id: "dark", label: "Dark", emoji: "🌙" }, { id: "light", label: "Light", emoji: "☀️" }, { id: "auto", label: "System", emoji: "💻" }].map(t => (
                    <button key={t.id} onClick={() => setTheme(t.id)} style={{ flex: 1, padding: "10px 12px", borderRadius: 8, border: `1px solid ${theme === t.id ? "rgba(212,83,75,0.3)" : "rgba(214,198,170,0.08)"}`, background: theme === t.id ? "rgba(212,83,75,0.1)" : "rgba(255,255,255,0.03)", color: theme === t.id ? "#D4534B" : "#7A7062", cursor: "pointer", fontSize: 13, fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
            ...
            <div style={S.card}>
              <h4 style={{ color: "#D6C6AA", fontFamily: "var(--font-display, 'Cormorant Garamond', serif)", fontSize: 16, margin: "0 0 14px" }}>Regional</h4>
        ...
        {/* Integrations */}
        <div style={S.card}>
          <h4 style={{ color: "#D6C6AA", fontFamily: "var(--font-display, 'Cormorant Garamond', serif)", fontSize: 18, margin: "0 0 16px" }}>Integrations</h4>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
            {[
              ...
              <div key={int.name} style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${int.connected ? "rgba(59,165,92,0.15)" : "rgba(214,198,170,0.06)"}`, borderRadius: 10, padding: 14, display: "flex", alignItems: "center", gap: 12 }}>
                <span style={{ fontSize: 22 }}>{int.icon}</span>
                <div style={{ flex: 1 }}>
                  <div style={{ color: "#D6C6AA", fontSize: 13, fontWeight: 600 }}>{int.name}</div>
                  <div style={{ color: "#5A5347", fontSize: 11 }}>{int.desc}</div>
                </div>
                <div style={{ width: 8, height: 8, borderRadius: "50%", background: int.connected ? "#3BA55C" : "#5A5347" }} />
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ ...S.card, border: "1px solid rgba(212,83,75,0.15)" }}>
          <h4 style={{ color: "#D4534B", fontFamily: "var(--font-display, 'Cormorant Garamond', serif)", fontSize: 16, margin: "0 0 12px" }}>Danger Zone</h4>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
            <div>
              <div style={{ color: "#D6C6AA", fontSize: 14 }}>Export All Data</div>
              <div style={{ color: "#5A5347", fontSize: 12 }}>Download a copy of all project data in JSON format</div>
            </div>
            <button style={{ ...S.btnG, color: "#D4903B", borderColor: "rgba(212,144,59,0.25)" }}>Export Data</button>
          </div>
          <div style={{ height: 1, background: "rgba(214,198,170,0.06)", margin: "14px 0" }} />
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
            <div>
              <div style={{ color: "#D6C6AA", fontSize: 14 }}>Reset Studio</div>
              <div style={{ color: "#5A5347", fontSize: 12 }}>This will clear all local data and reset to defaults</div>
            </div>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
          {showSaved && <span style={{ color: "#3BA55C", fontSize: 14, display: "flex", alignItems: "center", gap: 6, animation: "slideUp .3s ease" }}>✓ Settings saved</span>}
